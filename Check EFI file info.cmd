...
powershell -ExecutionPolicy Bypass -Command "Import-Module -Force '%~dp0\ps\Get-EfiSignatures.psm1'; $hashnsigs = Get-EfiSignatures -FilePath '%filepath%'; Write-Host \""Authenticode SHA256: $($hashnsigs.Authentihash)\""; $json = Get-Content -Path '%~dp0\dbx_info\dbx_info_msft_latest.json' -Raw | ConvertFrom-Json; $allImages = @($json.images.x64 + $json.images.x86 + $json.images.arm + $json.images.arm64); foreach ($image in $allImages) { if ($hashnsigs.Authentihash -eq $image.authenticodeHash) { Write-Warning \""This hash was added to the UEFI revocation list on $($image.dateOfAddition)\""; if ($image.description) { Write-Warning \""$($image.description)\""; } } } Write-Host \""`n`n\""; Write-Host 'Signature Certificate(s):'; if ($hashnsigs.Signatures) { foreach ($sig in $hashnsigs.Signatures) { $sig.Signer | Format-List } } else { \""No signatures found`n\"" }"
